...
const FacturacionScreen = () => {
  ...
  const agregarProductoAFactura = (producto, cantidad) => {
    ...
    setFacturaItems((prev) => [...prev, productoConCantidad]);
  };

  const eliminarProducto = (id) => {
    setFacturaItems((prev) => prev.filter(item => item.id !== id));
  };
  ...
  const calcularTotal = () => {
    const subtotal = facturaItems.reduce((total, item) => total + (item.precio * item.cantidad), 0);
  ...
  const enviarFacturaPorCorreo = async () => {
    ...
    const body = `
      ...
      Productos:
      ${facturaItems.map(item => `${item.nombre} (x${item.cantidad}): $${(item.precio * item.cantidad).toFixed(2)}`).join('\n')}
      Costos Extras:
    ...
    const options = {
      recipients: [email],
      subject: 'Factura de NexTech',
      body: body,
    };

    const result = await MailComposer.composeAsync(options);
    if (result.status === 'sent') {
      Alert.alert('Factura enviada', 'La factura ha sido enviada correctamente.');
      setFacturas(prev => [...prev, { items: facturaItems, costoExtras, total }]);
      setFacturaItems([]);
      ...
      Alert.alert('Error', 'No se pudo enviar la factura. Inténtalo de nuevo.');
    }
  };
  ...
  const irAHistorialFacturas = () => {

    navigation.navigate('HistorialFacturas');
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TextInput
        ...
        style={styles.input}
      />
      <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('SeleccionProductos', { agregarProductoAFactura })}>
      ...
      <TouchableOpacity style={styles.button} onPress={enviarFacturaPorCorreo}>
        <Text style={styles.buttonText}>Enviar Factura</Text>
      </TouchableOpacity>
      <TouchableOpacity style={[styles.button, styles.historialButton]} onPress={irAHistorialFacturas}>
        <Text style={styles.buttonText}>Historial de Facturas</Text>
      </TouchableOpacity>
    </ScrollView>
  );
};
